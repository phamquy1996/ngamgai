                ...
                <div class="card-body">
                    <table id="bootstrap-data-table" class="table table-striped table-bordered">
                        ...
                        @foreach($product as $item)
                            <tr>
                                <td>{{$item->title}}</td>
                                <td><img class="img-fluid" src="img/{{$item->image}}" alt="" width="150px"></td>
                                <td>{{$item->price}}</td>
                                <td>{{$item->discount}}</td>
                                <td>{{$item->thuonghieu}}</td>
                                <td>{{$item->title}}</td>
                                <td>
                                    <a href="quantri/products/edit/{{ $item->id }}" title="Sửa"><i class="fa fa-edit"></i></a>
                                    <a href="quantri/products/delete/{{ $item->id }}" title="Xóa"><i class="fa fa-trash-o"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
